...
{
    public partial class ComprasPorTipo : Form
    {
        AD_TipoTarjeta _DB = new AD_TipoTarjeta();
        int anioReporte = DateTime.Today.Year;
        int mesReporte = DateTime.Today.Month;
        public ComprasPorTipo()
        {
            InitializeComponent();
        ...
        {
            DataTable dt = _DB.GetComprasPorTipo(anioReporte.ToString(), mesReporte.ToString());
            if (dt.Rows.Count > 0)
            {
                ReportDataSource ds = new ReportDataSource("DatosTipoTarjeta", dt);
